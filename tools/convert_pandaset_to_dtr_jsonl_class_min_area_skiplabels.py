...
今回版の追加点:
- skip_label の内訳を label_stats.json / skip_label_stats.json / skip_label_stats.csv に出力
- class別 min_area に対応
  例:
...
def cuboids_to_objects(
    ...
    for _, row in df.iterrows():
        ...
        if cls is None:
            ...
            skip_label_counter[str(raw_label)] += 1
            continue
...
def main() -> None:
    ...
    skip_label_counter: Counter = Counter()
        ...
        for cam_name in cameras:
            ...
            for frame_idx in range(0, num_frames, max(1, args.frame_stride)):
                ...
                objects = cuboids_to_objects(
                    ...
                    skip_label_counter=skip_label_counter,
                )
    ...
    stats = {
        ...
        "skip_label_counter_raw": dict(skip_label_counter.most_common()),
    ...
    if not args.dry_run:
        ...
        # skip_label の内訳を単独ファイルにも出力
        with open(out_root / "skip_label_stats.json", "w", encoding="utf-8") as f:
            json.dump(dict(skip_label_counter.most_common()), f, ensure_ascii=False, indent=2)

        with open(out_root / "skip_label_stats.csv", "w", encoding="utf-8") as f:
            f.write("label,count\n")
            for label, count in skip_label_counter.most_common():
                safe_label = str(label).replace('"', '""')
                f.write(f'"{safe_label}",{count}\\n')
    ...
    print("skip_label_counter_raw:", dict(skip_label_counter.most_common()))
